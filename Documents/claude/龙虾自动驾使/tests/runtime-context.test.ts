...
function createServiceContext(overrides: Partial<OpenClawPluginServiceContext> = {}): OpenClawPluginServiceContext {
  return {
    config: {},
...
test("RuntimeContextManager.requireStateDir should throw when no state dir", () => {
  ...
  assert.throws(
    () => manager.requireStateDir(),
    /OpenClaw stateDir 缺失/,
  );
});
...
test("RuntimeContextManager.getRuntimeContextFromCommand should merge command config", () => {
  const manager = new RuntimeContextManager({ logger: createLogger() });
  const runtimeCtx = createServiceContext({ config: { base: true, override: "runtime" } });
...
test("RuntimeContextManager.clearRuntimeContext should allow re-initialization", () => {
  ...
  manager.rememberRuntimeContext(createServiceContext({ workspaceDir: "/first" }));
  manager.clearRuntimeContext();
  manager.rememberRuntimeContext(createServiceContext({ workspaceDir: "/second" }));
