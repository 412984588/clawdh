...
function createStateContext(workspaceDir: string, stateDir: string): OpenClawPluginServiceContext {
  return {
    config: {},
    workspaceDir,
    stateDir,
...
test("StatePersistenceManager should return error stats by category", () => {
  ...
  manager.addError({ loop: 1, category: "file_io", message: "e1", timestamp: Date.now() });
  manager.addError({ loop: 2, category: "file_io", message: "e2", timestamp: Date.now() });
  manager.addError({ loop: 3, category: "network", message: "e3", timestamp: Date.now() });
...
test("StatePersistenceManager should compress context to max limits", () => {
  ...
  // Add 10 actions
  for (let i = 0; i < 10; i++) {
    manager.addAction({ loop: i, action: `a${i}`, result: "ok", timestamp: Date.now() });
  }
  // Add 8 errors
  for (let i = 0; i < 8; i++) {
    manager.addError({ loop: i, category: "test", message: `e${i}`, timestamp: Date.now() });
  }
...
test("StatePersistenceManager should not compress when under limits", () => {
  ...
  manager.addAction({ loop: 1, action: "a1", result: "ok", timestamp: Date.now() });
  manager.addAction({ loop: 2, action: "a2", result: "ok", timestamp: Date.now() });
...
test("StatePersistenceManager.persistState should write state file atomically", async () => {
  await withTempWorkspace("lobster-state-persist", async ({ stateDir }) => {
    ...
    manager.addAction({ loop: 5, action: "test", result: "ok", timestamp: Date.now() });

    await manager.persistState(ctx);

    const statePath = path.join(stateDir, StateFileNames.ENGINE_STATE);
    const data = JSON.parse(await fs.readFile(statePath, "utf-8")) as EngineState;
...
test("StatePersistenceManager.persistState should not leave temp files", async () => {
  await withTempWorkspace("lobster-state-no-temp", async ({ stateDir }) => {
    ...
    const tmpPath = path.join(stateDir, StateFileNames.ENGINE_STATE + StateFileNames.TEMP_SUFFIX);
  ...
  await withTempWorkspace("lobster-state-recover", async ({ stateDir }) => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        loopCount: 42,
        context: {
          actions: [{ loop: 42, action: "persisted", result: "ok", timestamp: 1000 }],
          errors: [{ loop: 40, category: "test", message: "old error", timestamp: 500 }],
        },
...
test("StatePersistenceManager should roundtrip persist and recover", async () => {
  await withTempWorkspace("lobster-state-roundtrip", async ({ stateDir }) => {
    ...
    manager1.addAction({ loop: 100, action: "roundtrip test", result: "success", timestamp: Date.now() });
    manager1.addError({ loop: 99, category: "timeout", message: "request timeout", timestamp: Date.now() });
