...
export class LobsterCodeFixer {
  ...
  private async fixFile(filePath: string): Promise<FixResult[]> {
    ...
    try {
      ...
      const fixes: Array<{ node: ts.Node; type: FixType; range: [number, number] }> = [];
      ...
      const collectFixes = (node: ts.Node) => {
        ...
        if (this.config.enabledFixes.includes(FixType.REMOVE_CONSOLE_LOG)) {
          if (this.isConsoleLog(node)) {
            ...
            fixes.push({ node, type: FixType.REMOVE_CONSOLE_LOG, range: [start, end] });
          }
        }
        ...
        if (this.config.enabledFixes.includes(FixType.FIX_EMPTY_CATCH)) {
          ...
          if (emptyCatch) {
            const start = node.getStart(sourceFile);
            const end = node.getEnd();
            fixes.push({ node, type: FixType.FIX_EMPTY_CATCH, range: [start, end] });
          }
        }
        ...
        if (this.config.enabledFixes.includes(FixType.REMOVE_ANY_TYPE)) {
          if (this.isAnyType(node)) {
            const start = node.getStart(sourceFile);
            const end = node.getEnd();
            fixes.push({ node, type: FixType.REMOVE_ANY_TYPE, range: [start, end] });
          }
        }
      ...
      for (const fix of fixes) {
        const fixResult = await this.applyFix(sourceFile, fix, modifiedCode, filePath);
      ...
      if (successfulFixes.length > 0 && results.some(r => r.afterCode !== undefined)) {
