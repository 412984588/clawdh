...
export class HTNPlanner {
  ...
  registerDomain(domain: HTNDomain): void {
    ...
    console.log(`🦞 注册域: ${domain.name} (${domain.methods.length} 方法, ${domain.operators.size} 操作符)`);
  }
  ...
  async plan(
    ...
    console.log(`   配置: maxDepth=${this.config.maxDepth}, parallel=${this.config.enableParallel}, backtracking=${this.config.enableBacktracking}`);

    // 获取域
    const domain = domainName ? this.domains.get(domainName) : this.getDefaultDomain();
    if (!domain) {
    ...
    // 提取待执行任务
    const pendingTasks = allTasks.filter(t => t.status === TaskStatus.PENDING && t.type === HTNTaskType.PRIMITIVE);
  ...
  private async decomposeTask(
    ...
    if (currentDepth >= this.config.maxDepth) {
      console.warn(`⚠️ 达到最大深度 ${this.config.maxDepth}，停止分解: ${task.name}`);
    ...
    console.log(`🔧 分解: ${task.name} -> ${task.subtasks.map(t => t.name).join(", ")}`);
  ...
  async execute(
    ...
    for (const task of sortedTasks) {
      if (task.type === HTNTaskType.PRIMITIVE && task.status === TaskStatus.PENDING) {
        ...
        try {
          ...
          console.log(`✅ 完成: ${task.name} (${task.endTime - task.startTime!}ms)`);
        } catch (error) {
    ...
    // 更新根任务状态
    const rootFailed = sortedTasks.some(t => t.status === TaskStatus.FAILED && !this.isAncestorCompleted(t, sortedTasks));
    plan.rootTask.status = rootFailed ? TaskStatus.FAILED : TaskStatus.COMPLETED;
  ...
  exportPlanText(plan: HTNPlan): string {
    ...
    const printTask = (task: HTNTask, indent: string = "") => {
      ...
      lines.push(`${indent}${statusIcon} ${typeIcon} ${task.name} [${task.type}]`);
...
export const GENERAL_DOMAIN: HTNDomain = {
  ...
  operators: new Map<string, (params: any) => Promise<any>>([
    ["log", async (params) => { console.log(params?.message || ""); return { logged: true }; }],
    ["sleep", async (params) => {
