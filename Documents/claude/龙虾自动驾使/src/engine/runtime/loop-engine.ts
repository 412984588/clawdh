...
export class LoopEngineManager {
  ...
  constructor(
    ...
    private readonly dependencies: {
      loadMissionFiles: (ctx: OpenClawPluginServiceContext) => Promise<{ mission: string; boundaries: string }>;
      planNextAction: (mission: string, context: ContextState) => Promise<{
        description: string;
        type: string;
  ...
  private async runLoop(
    ...
    this.state.loopMetrics = { totalTime: 0, minTime: Infinity, maxTime: 0, avgTime: 0 };
    ...
    while (this.state.isRunning && !signal.aborted) {
      ...
      try {
        ...
        // 2. 生成下一步行动
        const action = await this.dependencies.planNextAction(mission, this.state.context);
        ...
        // 5. 定期压缩上下文
        if (this.state.loopCount % this.config.compressInterval === 0 && this.state.loopCount > 0) {
          this.dependencies.compressContext();
        }

        // 6. 定期清理过期缓存
        if (this.state.loopCount % (this.config.compressInterval * 2) === 0 && this.state.loopCount > 0) {
    ...
    this.api.logger.info(`🔄 永动循环结束，总循环次数: ${this.state.loopCount}`);
  ...
  private recordLoopTime(): void {
    ...
    this.state.loopMetrics.avgTime = Math.round(
  ...
  private logPerformanceMetrics(): void {
    const minTime = this.state.loopMetrics.minTime === Infinity ? 0 : this.state.loopMetrics.minTime;
  ...
  private categorizeError(errorMsg: string): ErrorCategory {
    ...
    const rules = [
      { patterns: ["enoent", "eacces", "file"], category: ErrorCategory.FILE_IO },
      { patterns: ["syntax", "parse", "json"], category: ErrorCategory.PARSE },
      { patterns: ["network", "fetch", "request"], category: ErrorCategory.NETWORK },
      { patterns: ["permission", "unauthorized", "forbidden"], category: ErrorCategory.PERMISSION },
  ...
  private startHealthCheck(ctx?: OpenClawPluginServiceContext): void {
    ...
    this.healthCheckInterval = setInterval(async () => {
      ...
      if (timeSinceLastLoop > this.config.stallThreshold && this.state.isRunning) {
        this.api.logger.warn(
          `⚠️ 循环已卡死 ${stallSeconds} 秒`
        );
