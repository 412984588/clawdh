...
export class ReActAgent {
  ...
  async execute(
    ...
    while (iteration < this.config.maxIterations && this.currentState !== "done") {
      ...
      // 自我反思
      if (this.config.enableSelfReflection && iteration > 0 && iteration % 3 === 0) {
...
Recent Memory:
${this.memory.slice(-5).map(m => `- Thought: ${m.thought}\n  Action: ${m.action}\n  Result: ${m.result}`).join("\n\n")}
...
`.trim();
   ...
   * 生成思考（模拟 LLM）
   */
  private async generateThought(prompt: string, iteration: number): Promise<string> {
    // 实际应用中这里会调用 LLM API
    // 现在返回一个模拟的思考过程
   ...
   * 解析行动
   */
  private parseAction(thought: string): { action?: string; actionInput?: Record<string, any> } {
    // 简单解析，实际应用中需要更复杂的解析
  ...
  exportThoughtChain(): string {
    return this.thoughts
      .map((step, i) =>
        ...
        `  Thought: ${step.thought}\n` +
        (step.action ? `  Action: ${step.action}(${JSON.stringify(step.actionInput)})\n` : "") +
