...
export class IncrementalLearningEngine {
  ...
  recordExperience(entry: Omit<ExperienceEntry, "id">): void {
    ...
    if (this.shortTermMemory.length > this.config.shortTermMemorySize) {
      ...
      // 重要经验转移到长期记忆
      if (removed && ((removed.accessCount ?? 0) > 3 || (removed.successRate && removed.successRate > 0.7))) {
    ...
    console.log(`📝 记录经验: ${entry.taskType} (短期: ${this.shortTermMemory.length}, 长期: ${this.longTermMemory.length})`);
  }
    ...
    const relevant = allExperiences
      ...
      .filter(exp => {
        ...
        return contextKeys.some(key =>
          exp.context?.[key] === currentContext[key]
        );
      })
      .sort((a, b) => {
        // 优先级：成功率 > 访问次数 > 时间
        const aScore = (a.successRate || 0) * 100 + (a.accessCount ?? 0) + (1000000 - (a.timestamp ?? 0)) / 1000000;
        const bScore = (b.successRate || 0) * 100 + (b.accessCount ?? 0) + (1000000 - (b.timestamp ?? 0)) / 1000000;
  ...
  async learn(
    ...
    // 更新性能指标
    const currentPerformance = this.learningState.performanceMetrics.get(taskType) || 0.5;
    ...
    console.log(`🧠 学习完成: ${taskType} (性能: ${newPerformance.toFixed(2)})`);
  }
  ...
  private async adapt(taskType: string): Promise<void> {
    ...
    console.log(`🔄 触发适应: ${taskType} (第 ${this.learningState.adaptationCount} 次)`);
  ...
  getBestAction(
    ...
    const performance = this.learningState.performanceMetrics.get(taskType) || 0.5;
    ...
    return actionScores[0] || { action: availableActions[0] || "default", confidence: 0.5 };
  }
  ...
  private getExperienceBonus(taskType: string, action: string): number {
    ...
    // 计算平均成功率
    const avgSuccess = relevant.reduce((sum, e) => sum + (e.successRate || 0), 0) / relevant.length;
    return avgSuccess;
  }
...
=== 最近错误 ===
${state.recentErrors.slice(-5).map(e => `- ${e}`).join("\n")}
` : ""}
    `.trim();
  }
  ...
  compressMemories(): number {
    ...
    this.shortTermMemory = this.shortTermMemory.filter(exp => {
      // 保留高访问次数或高成功率的经验
      return (exp.accessCount ?? 0) > 1 || (exp.successRate && exp.successRate > 0.6);
    });
    ...
    if (this.learningState.recentErrors.length > 50) {
      ...
      this.learningState.recentErrors = this.learningState.recentErrors.slice(-20);
  ...
  importModel(modelData: Record<string, any>): void {
    ...
    if (modelData.learningState) {
      this.learningState = {
        ...
        performanceMetrics: new Map(Object.entries(modelData.learningState.performanceMetrics)),
        recentErrors: modelData.learningState.recentErrors || [],
        adaptationCount: modelData.learningState.adaptationCount || 0,
        lastAdaptationTime: modelData.learningState.lastAdaptationTime || Date.now(),
      };
    }
