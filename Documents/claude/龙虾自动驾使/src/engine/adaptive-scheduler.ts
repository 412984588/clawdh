...
export class AdaptiveScheduler {
  ...
  schedule(task: Omit<ScheduledTask, "id" | "createdAt" | "attempts">): string {
    ...
    console.log(`🦞 任务已调度: ${task.name} (优先级: ${TaskPriority[scheduledTask.priority]})`);
  ...
  getNextTask(): ScheduledTask | undefined {
    ...
    for (let i = 0; i < this.queue.length; i++) {
      ...
      // 检查是否需要降级
      if (this.isDegraded && task.canDegrade && task.priority > TaskPriority.CRITICAL) {
  ...
  async executeNext(): Promise<boolean> {
    ...
    try {
      ...
      if (task.attempts < task.maxRetries) {
        ...
        this.sortQueue();
        console.warn(`⚠️ 任务失败，将重试: ${task.name} (${task.attempts}/${task.maxRetries})`);
      } else {
   ...
   * 设置任务完成回调
   */
  setTaskCompleteCallback(callback: (taskId: string, result: any) => void): void {
   ...
   * 设置任务失败回调
   */
  setTaskFailedCallback(callback: (taskId: string, error: Error) => void): void {
