...
export class AgentFormatParser {
  ...
  static validate(card: any): AgentCard {
    ...
    if (!card.interface?.input?.contentType || !card.interface?.output?.contentType) {
      throw new Error("Agent interface must include input and output contracts");
    }
  ...
  static serializeYaml(card: AgentCard): string {
    ...
    if (card.constraints?.predicates) {
      ...
      card.constraints.predicates.forEach((p, i) => {
        ...
        if (p.allowedValues) lines.push(`      allowedValues: ${JSON.stringify(p.allowedValues)}`);
        if (p.deniedValues) lines.push(`      deniedValues: ${JSON.stringify(p.deniedValues)}`);
      });
      lines.push("");
    }
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (Array.isArray(value)) {
        ...
        value.forEach((item, i) => {
          const itemStr = typeof item === "object" ? JSON.stringify(item, null, 0) : String(item);
  ...
  private static evaluateConstraint(
    ...
    switch (constraint.type) {
      case ConstraintType.READ_ONLY:
        ...
        const hasWriteKeyword = writeKeywords.some(kw => action.toUpperCase().includes(kw));
        if (hasWriteKeyword) {
          return { allowed: false, reason: "Contains write operation (read-only constraint)" };
        }
        break;
      ...
      case ConstraintType.TOKEN_BUDGET:
        ...
        if (constraint.expression && estimatedTokens > parseInt(constraint.expression)) {
          return { allowed: false, reason: `Exceeds token budget (${estimatedTokens} > ${constraint.expression})` };
        }
        break;
    ...
    if (constraint.allowedValues && !constraint.allowedValues.includes(action)) {
    ...
    },
    {
        ...
        input: {
          ...
          jsonSchema: {
            ...
            properties: {
              ...
              review_focus: {
                type: "array",
                items: { type: "string", enum: ["correctness", "style", "security"] },
              },
            },
            required: ["pr_url"],
          },
      ...
      executionPolicy: {
        ...
        config: {
          provider: "anthropic",
          model: "claude-sonnet-4",
          instructions: "Review the code for correctness, style, and security issues.",
          maxSteps: 15,
          temperature: 0.3,
          toolChoice: "required",
        },
