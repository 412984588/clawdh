    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        if (options?.logResult !== false) {
          console.log(
            `📤 ${className}.${String(propertyKey)} 返回:`,
            result
          );
        }
        ...
        if (options?.logTiming) {
          ...
          console.log(`⏱️️ ${className}.${String(propertyKey)} 耗时: ${duration}ms`);
        }
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        const validator = ReflectedMetadata.getMetadata<{ rules?: Record<number, (value: any) => boolean | string> }>(
          target,
          key
        );
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (cached && cached.expiry > Date.now()) {
        console.log(`💾 缓存命中: ${target.constructor.name}.${String(propertyKey)}`);
        return cached.value;
      }
 ...
 * 获取度量统计
 */
export function getMetrics(target: object, methodName: string): {
  count: number;
  avgDuration: number;
  minDuration: number;
  maxDuration: number;
 ...
 * 创建装饰器组合器
 */
export function createDecoratorComposer(...decorators: ClassDecorator[]): ClassDecorator {
  return function (target: any) {
