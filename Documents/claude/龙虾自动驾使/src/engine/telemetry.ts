...
export class TelemetryCollector {
  private config: Required<Omit<TelemetryConfig, "format">> & { format: "json" | "prometheus" | "statsd" };
  ...
  constructor(config: Partial<TelemetryConfig> = {}) {
    ...
    } as Required<Omit<TelemetryConfig, "format">> & { format: "json" | "prometheus" | "statsd" };
  ...
  private convertToPrometheus(metrics: TelemetryMetrics[]): string {
    ...
    for (const metric of metrics) {
      // 事件循环指标
      if (metric.eventLoop) {
        lines.push(`# HELP lobster_event_loop_lag Event loop lag in milliseconds`);
        lines.push(`# TYPE lobster_event_loop_lag gauge`);
        lines.push(`lobster_event_loop_lag{quantile="avg"} ${metric.eventLoop.avgLag} ${metric.timestamp}`);
        lines.push(`lobster_event_loop_lag{quantile="p95"} ${metric.eventLoop.p95Lag || 0} ${metric.timestamp}`);
        lines.push(`lobster_event_loop_health_status{status="${metric.eventLoop.healthStatus}"} 1 ${metric.timestamp}`);
      }

      // 系统指标
      if (metric.system) {
        lines.push(`lobster_system_cpu_usage ${metric.system.cpuUsage} ${metric.timestamp}`);
        lines.push(`lobster_system_memory_usage ${metric.system.memoryUsage.heapUsed} ${metric.timestamp}`);
      }

      // 调度器指标
      if (metric.scheduler) {
        lines.push(`lobster_scheduler_queue_length ${metric.scheduler.queueLength} ${metric.timestamp}`);
        lines.push(`lobster_scheduler_running_tasks ${metric.scheduler.runningCount} ${metric.timestamp}`);
        lines.push(`lobster_scheduler_load_level{level="${metric.scheduler.loadLevel}"} 1 ${metric.timestamp}`);
      }
    }
  ...
  private convertToStatsD(metrics: TelemetryMetrics[]): string {
    ...
    for (const metric of metrics) {
      if (metric.eventLoop) {
        ...
        lines.push(`lobster.event_loop.lag.p95:${metric.eventLoop.p95Lag || 0}|ms`);
      }

      if (metric.system) {
        lines.push(`lobster.system.cpu:${metric.system.cpuUsage}|gauge`);
        lines.push(`lobster.system.memory.heap:${metric.system.memoryUsage.heapUsed}|bytes`);
      }
    }
