...
export class AutonomousTaskPlanner {
   ...
   */
  async planNextAction(ctx: OpenClawPluginServiceContext): Promise<PlanningResult> {
  ...
  private performSafetyCheck(): PlanningResult | null {
    ...
    if (recentFailures.length >= 5) {
      ...
      return {
        ...
        reasoning: "最近1分钟内有5次失败，根据 fail-safe 原则，暂停执行以避免级联失败",
        confidence: 0.95,
      };
    }
  ...
  private planErrorRecovery(): PlanningResult | null {
    const unresolvedErrors = this.context.errorHistory.filter(e => !e.resolved);
  ...
  private planContextCompression(): PlanningResult | null {
    const totalTasks = this.context.completedTasks.length + this.context.activeTasks.length;
   ...
   * 基于当前目标和状态，生成下一步行动
   */
  private planGoalOrientedTask(ctx: OpenClawPluginServiceContext): PlanningResult | null {
  ...
  private needsWorkspaceAnalysis(): boolean {
    ...
    const lastAnalysis = this.context.activeTasks.find(
      t => t.type === TaskType.ANALYZE && t.description.includes("分析工作区")
    );
