...
export class ChainOfThought {
   ...
   * 分析问题
   */
  private async analyze(question: string, context: string | undefined): Promise<void> {
  ...
  private async generateAnswer(
    question: string,
    context: string | undefined
  ): Promise<{ answer: string; confidence: number; verificationPassed: boolean }> {
    // 综合思维链生成答案
  ...
  private synthesizeAnswer(): string {
    // 从思维链中提取关键信息
    const keySteps = this.chain.filter(s =>
      s.action === "方案设计" ||
      s.action === "修正方案" ||
      s.action === "验证完成"
    );
  ...
  private calculateConfidence(): number {
    ...
    // 验证通过
    const hasVerification = this.chain.some(s =>
      s.action === "验证完成" && s.thought.includes("✅")
    );
...
export class TreeOfThought extends ChainOfThought {
  ...
  async reason(
    ...
    // 生成多个分支假设
    const branches = await this.generateBranches(question, context, maxBranches);
...
export function withCoT(
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      // 构造问题
      const question = description || `${propertyKey} 被调用，参数: ${JSON.stringify(args)}`;
