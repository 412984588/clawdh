...
export class Notifier {
   ...
   * 请求人工介入
   */
  async requestManual(title: string, content: string, fields?: Array<{ name: string; value: string }>): Promise<void> {
    return this.notify({
  ...
  private async sendToTelegram(message: NotificationMessage): Promise<void> {
    ...
    const { botToken, chatId, apiBaseUrl = "https://api.telegram.org" } = telegramConfig;
