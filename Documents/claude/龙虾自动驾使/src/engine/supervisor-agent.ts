...
export interface SupervisorEvent {
  /** 事件类型 */
  type: "agent_started" | "agent_completed" | "agent_failed" | "task_assigned" | "task_completed" | "task_failed";
  /** 代理ID */
  agentId?: string;
  /** 任务ID */
  taskId?: string;
...
export class SupervisorAgent {
  ...
  private assignmentStrategy: AssignmentStrategy = AssignmentStrategy.LEAST_LOADED;
  private eventListeners: Map<string, (event: SupervisorEvent) => void> = new Map();
  ...
  getStatus(): SupervisorStatus {
    ...
    const avgTaskDuration = totalTasks > 0
      ? completedTaskList.reduce((sum, t) => sum + (t.completedAt! - t.startedAt!), 0) / totalTasks
      : 0;
  ...
  private assignTasks(): void {
    ...
    const availableAgents = Array.from(this.agents.values())
      .filter(a => a.status === AgentStatus.IDLE && a.config.enabled);
  ...
  private selectTask(agent: AgentInstance): AgentTask | undefined {
    switch (this.assignmentStrategy) {
      case AssignmentStrategy.PRIORITY_FIRST:
        return this.taskQueue.find(t => !t.dependencies || t.dependencies.length === 0);

      case AssignmentStrategy.LEAST_LOADED:
        return this.taskQueue[0];

      case AssignmentStrategy.ROUND_ROBIN:
        return this.taskQueue[Math.floor(Math.random() * this.taskQueue.length)];
   ...
   * 分配任务给代理
   */
  private async assignTaskToAgent(agent: AgentInstance, task: AgentTask): Promise<void> {
    // 从队列移除
  ...
  private async executeTask(agent: AgentInstance, task: AgentTask): Promise<void> {
    try {
      ...
      agent.performanceData.avgTaskDuration =
        (agent.performanceData.avgTaskDuration * (agent.completedTasks - 1) + duration) / agent.completedTasks;
      ...
      if (task.retries < this.config.maxRetries) {
        task.retries++;
        console.log(`🦞 重试任务 ${task.id} (${task.retries}/${this.config.maxRetries})`);
    ...
    for (const agent of this.agents.values()) {
      if (agent.status === AgentStatus.RUNNING && agent.currentTask) {
        ...
        if (task.timeout && elapsed > task.timeout) {
          console.warn(`🦞 任务 ${task.id} 超时 (${elapsed}ms > ${task.timeout}ms)`);
