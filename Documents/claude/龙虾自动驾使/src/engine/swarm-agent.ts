...
export class SwarmAgent {
  ...
  // 任务处理函数映射
  private taskHandlers: Map<string, (task: SwarmTask, member: SwarmMemberState) => Promise<void>> = new Map();
  ...
  private assignRole(index: number): SwarmMemberState["role"] {
    const roles: SwarmMemberState["role"][] = ["worker", "worker", "worker", "worker", "worker", "scout", "scout", "coordinator", "specialist", "specialist"];
  ...
  private updateNeighbors(): void {
    for (const member of this.members.values()) {
      ...
      for (const other of this.members.values()) {
        ...
        const distance = this.calculateDistance(member.position, other.position);
   ...
   * 添加任务
   */
  addTask(task: Omit<SwarmTask, "id" | "createdAt" | "progress" | "status">): string {
  ...
  private assignTask(task: SwarmTask): void {
    ...
    if (bestMember) {
      ...
      task.assignedTo?.add(bestMember.id) ?? (task.assignedTo = new Set([bestMember.id]));
  ...
  private findBestMember(task: SwarmTask): SwarmMemberState | undefined {
    ...
    for (const member of this.members.values()) {
      ...
      // 检查能力要求
      if (task.requiredCapability && member.capability < task.requiredCapability) {
        continue;
      }
   ...
   * 计算适配度分数
   */
  private calculateFitnessScore(member: SwarmMemberState, task: SwarmTask): number {
    let score = 0;
    ...
    if (task.data?.position) {
      const distance = this.calculateDistance(member.position, task.data.position);
      score += Math.max(0, 1 - distance / this.config.communicationRadius) * 0.2;
    } else {
      score += 0.2;
    }

    // 邻居推荐 (10%)
    const neighborRecommendations = Array.from(member.neighbors).filter(id => {
      const neighbor = this.members.get(id);
      return neighbor && neighbor.status === "busy" && neighbor.currentTask === task.id;
    }).length;
   ...
   * 执行任务
   */
  private async executeTask(task: SwarmTask, member: SwarmMemberState): Promise<void> {
    try {
   ...
   * 默认任务处理器
   */
  private async defaultTaskHandler(task: SwarmTask, member: SwarmMemberState): Promise<void> {
    // 模拟任务执行
   ...
   * 发送消息给特定成员
   */
  sendToMember(memberId: string, message: Omit<AgentMessage, "id" | "timestamp">): void {
   ...
   * 投递消息
   */
  private deliverMessage(message: AgentMessage, member: SwarmMemberState): void {
    // 检查TTL
   ...
   * 处理协调消息
   */
  private handleCoordination(message: AgentMessage, member: SwarmMemberState): void {
      ...
      for (const neighborId of member.neighbors) {
        ...
        if (neighbor && neighbor.status === "idle" && neighbor.load < 0.5) {
          ...
          for (const task of this.tasks.values()) {
            if (task.assignedTo?.has(member.id) && task.status === "in_progress") {
              // 转移任务
              ...
              console.log(`🦞 任务 ${task.id} 从 ${member.id} 转移到 ${neighborId}`);
              break;
            }
          }
          break;
   ...
   * 更新成员位置
   */
  private updateMemberPosition(member: SwarmMemberState, position: { x: number; y: number; z: number }): void {
    member.position = position;
    this.updateNeighbors();
  }
  ...
  private autoScale(): void {
    const activeMembers = Array.from(this.members.values()).filter(m => m.status === "active" || m.status === "busy").length;
    const avgLoad = Array.from(this.members.values()).reduce((sum, m) => sum + m.load, 0) / this.members.size;
    const pendingTasks = Array.from(this.tasks.values()).filter(t => t.status === "pending").length;

    // 负载高且待处理任务多 -> 扩容
    if (avgLoad > 0.7 && pendingTasks > 3 && this.members.size < this.config.maxMembers) {
      this.addMember();
    }
    // 负载低且成员数多 -> 缩容
  ...
  private addMember(): void {
    ...
    console.log(`🦞 群体扩容: 新成员 ${newId} (当前 ${this.members.size} 个成员)`);
  }
  ...
  private removeMember(): void {
    ...
    if (candidate && this.members.size > this.config.minMembers) {
      ...
      console.log(`🦞 群体缩容: 移除成员 ${candidate.id} (当前 ${this.members.size} 个成员)`);
    }
  }
  ...
  getStatistics(): SwarmStatistics {
    ...
    const activeMembers = members.filter(m => m.status === "active" || m.status === "busy").length;
    const idleMembers = members.filter(m => m.status === "idle").length;
    const avgLoad = members.reduce((sum, m) => sum + m.load, 0) / members.length;
    const avgCapability = members.reduce((sum, m) => sum + m.capability, 0) / members.length;
