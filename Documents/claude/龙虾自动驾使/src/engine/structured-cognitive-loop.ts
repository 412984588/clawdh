...
export class StructuredCognitiveLoop {
  ...
  constructor(config: SCLConfig = {}) {
    ...
    console.log(`🧠 SCL 引擎初始化 (模式: ${this.config.defaultReasoningMode})`);
  }
  ...
  async perceive(input: PerceptionInput): Promise<CognitiveState> {
    ...
    if (perceptionModule) {
      const perceptionResult = await perceptionModule.process(input, this.state);
  ...
  async retrieve(query: {
    ...
    for (const memoryType of sourceTypes) {
      const results = await this.searchMemory(query.query, strategy, memoryType);
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ 检索完成: ${limitedMemories.length} 条记忆 (${duration}ms)`);
    return result;
  }
  ...
  async reason(goal: string, context?: any): Promise<ReasoningResult> {
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ 推理完成: ${result.steps.length} 步, 置信度: ${result.confidence.toFixed(2)} (${duration}ms)`);
  ...
  async reflect(
    ...
    const duration = Date.now() - startTime;
    console.log(`✅ 反思完成: ${success ? "成功" : "失败"}, ${learnings.length} 条学习 (${duration}ms)`);
