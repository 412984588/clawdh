...
export interface FlameGraphNode {
  ...
  name: string;
  /** 函数类别 */
  category: "scripting" | "rendering" | "painting" | "system" | "idle" | "other";
  /** 开始时间（微秒） */
  start: number;
  /** 持续时间（微秒） */
  duration: number;
...
export class FlameGraphCollector {
   ...
   * 记录函数调用
   */
  recordFunction(name: string, category: FlameGraphNode["category"] = "other"): () => void {
    if (!this.isCollecting) {
      return () => {};
    }
  ...
  getHottestPath(): FlameGraphNode[] {
    const sortedNodes = [...this.samples].sort((a, b) => b.duration - a.duration);
   ...
   * 获取瓶颈函数
   */
  getBottlenecks(limit: number = 5): Array<{ name: string; totalTime: number; percentage: number }> {
  ...
  getSummary(): {
    ...
    const topFunctions = Array.from(this.functionStats.values())
      .sort((a, b) => b.totalTime - a.totalTime)
      .slice(0, 10)
      .map(({ name, totalTime, callCount }) => ({ name, totalTime, callCount }));
...
export function flameGraph(
  ...
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (collector) {
        return await collector.recordAsync(functionName, () => originalMethod.apply(this, args), category);
      } else {
 ...
 * 获取全局火焰图收集器
 */
export function getGlobalFlameGraphCollector(): FlameGraphCollector | undefined {
 ...
 * 启动全局火焰图收集
 */
export function startGlobalFlameGraphCollection(config?: FlameGraphConfig): FlameGraphCollector {
