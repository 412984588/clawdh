...
export class CostMonitor {
  ...
  record(
    ...
    const record: CostRecord = {
      type,
      amount,
      unit: type === CostType.EXECUTION_TIME ? "ms" : type === CostType.TOKEN ? "tokens" : "count",
      timestamp: Date.now(),
      source,
      metadata,
    };
  ...
  getStatistics(): CostStatistics {
    ...
    for (const record of this.records) {
      ...
      if (record.type === CostType.TOKEN) {
        ...
        stats.estimatedCost += (record.amount / 1000) * this.config.costPer1kTokens;
      ...
      if (record.source) {
        stats.bySource[record.source] = (stats.bySource[record.source] || 0) + record.amount;
      }
    }
  ...
  getBudgetStatus(): BudgetStatus {
    ...
    if (this.config.maxApiCalls && stats.totalApiCalls >= this.config.maxApiCalls) {
      return BudgetStatus.DEPLETED;
    }
    if (this.config.maxExecutionTime && stats.totalExecutionTime >= this.config.maxExecutionTime) {
      return BudgetStatus.DEPLETED;
    }

    // 检查是否警告
    if (this.config.maxTokens && stats.totalTokens >= this.config.maxTokens * this.config.tokenWarningThreshold) {
...
按来源:
${Object.entries(stats.bySource).map(([source, amount]) => `  ${source}: ${amount.toLocaleString()}`).join('\n')}
    `.trim();
  }
}
 ...
 * 创建成本监控器
 */
export function createCostMonitor(config?: Partial<CostBudgetConfig>): CostMonitor {
