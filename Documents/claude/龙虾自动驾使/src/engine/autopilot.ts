...
import { UltraThinkEngine, DEFAULT_ULTRATHINK_CONFIG, type UltraThinkConfig } from "./ultrathink.js";
import { RalphLoopEngine, DEFAULT_RALPH_CONFIG, type RalphLoopConfig, FixStrategy } from "./ralph-loop.js";
...
export class AutopilotController {
    ...
    try {
      ...
      const ralphResult = await this.ralphLoop.execute(
        ...
        async (attempt, strategy) => {
          ...
          this.log(`  📍 尝试 ${attempt}/${this.config.maxIterations} (策略: ${strategy})`);

          // 根据 UltraThink 的分析结果调整修复策略
          const enhancedPrompt = this.buildEnhancedPrompt(problem, thinkResult, attempt, strategy);
   ...
   * 获取思考路径
   */
  getThoughtPath(): Array<{ type: string; content: string; confidence: number }> {
   ...
   * 获取修复历史
   */
  getFixHistory(): Array<{ attempt: number; strategy: string; success: boolean; summary: string }> {
 ...
 * 创建 Autopilot 控制器
 */
export function createAutopilot(config?: Partial<AutopilotConfig>): AutopilotController {
 ...
 * 快速执行（使用默认配置）
 */
export async function quickAutopilot(problem: string): Promise<AutopilotSession> {
