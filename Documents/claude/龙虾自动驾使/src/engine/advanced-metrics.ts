...
export class AdvancedMetricsCollector {
  ...
  private calculateBackpressure(): number {
    ...
    if (p95Lag < this.config.p95LagWarning) {
      ...
      return 3 + (p95Lag - this.config.p95LagWarning) / (this.config.p95LagCritical - this.config.p95LagWarning) * 4;
    } else {
      return 7 + Math.min(3, (p95Lag - this.config.p95LagCritical) / this.config.p95LagCritical);
    }
  }
   ...
   * 计算健康状态
   */
  private calculateHealthStatus(metrics: AdvancedEventLoopMetrics): "healthy" | "degraded" | "unhealthy" {
   ...
   * 设置更新回调
   */
  setUpdateCallback(callback: (metrics: AdvancedEventLoopMetrics) => void): void {
