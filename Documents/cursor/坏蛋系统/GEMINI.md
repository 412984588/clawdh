...
📊 数据源管理策略
🎯 数据源类型（按优先级排序）
支付处理器目录：Stripe Connect、PayPal Partners等官方目录
创作者平台聚合器：Patreon替代品列表、Substack竞争者分析
众筹平台数据库：Kickstarter、Indiegogo类似平台
支付SaaS目录：Payment处理平台清单
开发者工具库：GitHub上的支付相关项目
行业报告白皮书：FinTech、创作者经济报告
社交媒体监测：Twitter、LinkedIn上的平台推荐
应用商店分析：App Store、Google Play中的支付应用
🔍 数据源获取方法
自动化搜索：使用关键词组合进行批量搜索
竞品分析：分析已知平台的竞争对手
API接口：利用公开API获取平台列表
...
🗂️ 文件管理系统
📁 必需文件清单
verified_platforms.json - 已验证通过的平台
rejected_platforms.json - 验证失败的平台
pending_platforms.json - 待验证平台队列
data_sources.json - 数据源清单和状态
validation_summary.md - 验证经验总结
strategy_updates.md - 策略变更记录
🔄 文件更新规则
...
📈 每轮循环的5个阶段
发现数据源: 自动扩展和优化数据源池
发现平台: Scout Agent智能生成候选
潜力分析: Analyzer Agent评估和排序
严格验证: Validator Agent执行4项验证
智能优化: 基本轮结果优化下一轮策略
🧠 自我优化机制
性能监控:
通过率监控 (低于15%时自动调整)
...
🔄 自优化工作流
智能发现: CrewAI编排的并行探索，数据源隔离
状态分析: LangGraph状态机，特征提取和ML评分
对话验证: AutoGen多轮对话，质量控制
智能汇总: CrewAI协调，自动参数优化
...
快速启动:
python3 claude_auto_workflow_v2.py --test  # V2测试模式
python3 claude_auto_workflow_v2.py --workers=5 --batch-size=50  # V2正式运行
python3 claude_auto_workflow.py --batch  # V1基础版

工作流触发: 在Claude Code中使用 /auto-discover

技术栈: Python + Exa API + Playwright + 并行处理 + 深度交互
