"""Tests for fastify-patterns-dev bundle."""
import json
import zipfile
...
def test_example_files_contain_fastify():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.ts").read_text()
        assert "fastify" in content.lower() or "Fastify" in content, \
            f"example.ts in {tid} missing Fastify reference"
...
def test_starter_zip_exists():
    assert (DIST / "fastify-patterns-starter-v1.0.0.zip").exists()


def test_pro_zip_exists():
    assert (DIST / "fastify-patterns-pro-v1.0.0.zip").exists()


def test_starter_zip_has_root_files():
    z = zipfile.ZipFile(DIST / "fastify-patterns-starter-v1.0.0.zip")
    names = z.namelist()
...
def test_pro_zip_has_all_templates():
    z = zipfile.ZipFile(DIST / "fastify-patterns-pro-v1.0.0.zip")
    names = z.namelist()
    for tid in TEMPLATE_IDS:
