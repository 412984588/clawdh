"""Tests for playwright-e2e-patterns-dev bundle."""
import json
import zipfile
...
def test_example_files_exist():
    for tid in TEMPLATE_IDS:
        assert (TEMPLATES / tid / "example.test.ts").exists(), f"Missing example.test.ts in {tid}"


def test_example_files_contain_playwright():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.test.ts").read_text()
        assert "playwright" in content.lower() or "test(" in content or "expect(" in content, \
            f"example.test.ts in {tid} missing Playwright reference"


def test_example_files_have_code():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.test.ts").read_text()
        assert "test(" in content or "describe(" in content or "it(" in content, \
            f"example.test.ts in {tid} has no test blocks"


def test_example_files_not_empty():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.test.ts").read_text().strip()
        assert len(content) > 200, f"example.test.ts in {tid} too short ({len(content)} chars)"
...
def test_starter_zip_exists():
    assert (DIST / "playwright-e2e-patterns-starter-v1.0.0.zip").exists()


def test_pro_zip_exists():
    assert (DIST / "playwright-e2e-patterns-pro-v1.0.0.zip").exists()


def test_starter_zip_has_root_files():
    z = zipfile.ZipFile(DIST / "playwright-e2e-patterns-starter-v1.0.0.zip")
    names = z.namelist()
...
def test_pro_zip_has_all_templates():
    z = zipfile.ZipFile(DIST / "playwright-e2e-patterns-pro-v1.0.0.zip")
    names = z.namelist()
    for tid in TEMPLATE_IDS:
