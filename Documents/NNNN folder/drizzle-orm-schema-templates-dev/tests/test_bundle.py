"""
Tests for Drizzle ORM Schema Templates Pack. TDD — RED first.
"""
...
def test_each_template_has_schema_ts():
    for name in TEMPLATE_DIRS:
        assert (TEMPLATES_DIR / name / "schema.ts").exists(), f"{name}: missing schema.ts"

def test_each_template_has_readme():
    for name in TEMPLATE_DIRS:
        assert (TEMPLATES_DIR / name / "README.md").exists(), f"{name}: missing README.md"

def test_schema_ts_not_empty():
    for name in TEMPLATE_DIRS:
        size = (TEMPLATES_DIR / name / "schema.ts").stat().st_size
        assert size > 100, f"{name}: schema.ts too small ({size} bytes)"

def test_schema_imports_drizzle():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "schema.ts").read_text()
        assert "drizzle-orm" in content, f"{name}: schema.ts doesn't import drizzle-orm"

def test_schema_has_pgTable_or_sqliteTable():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "schema.ts").read_text()
        has_pg = "pgTable" in content
        has_sqlite = "sqliteTable" in content
        assert has_pg or has_sqlite, f"{name}: schema.ts has no pgTable or sqliteTable"

def test_schema_exports_tables():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "schema.ts").read_text()
        assert "export const " in content, f"{name}: schema.ts has no exports"
