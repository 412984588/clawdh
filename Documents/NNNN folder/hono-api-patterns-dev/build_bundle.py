"""Build distribution ZIPs for hono-api-patterns-dev."""
...
def build_zip(tier: dict) -> None:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for fname in ROOT_FILES:
            ...
            if src.exists():
                zf.write(src, fname)
        for tpl_id in tier["templates"]:
            tpl_dir = TEMPLATES / tpl_id
            if not tpl_dir.is_dir():
                print(f"  WARNING: missing {tpl_id}")
                continue
            for fpath in sorted(tpl_dir.rglob("*")):
                if fpath.is_file():
                    zf.write(fpath, f"templates/{tpl_id}/{fpath.relative_to(tpl_dir)}")
