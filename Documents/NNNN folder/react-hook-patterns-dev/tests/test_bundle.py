"""
Tests for React Hook Patterns. TDD — RED first.
"""
...
TEMPLATE_DIRS = [
    "01-use-debounce",
    "02-use-local-storage",
    "03-use-media-query",
    "04-use-intersection-observer",
    "05-use-fetch",
    "06-use-previous",
    "07-use-clipboard",
    "08-use-event-listener",
    "09-use-key-press",
    "10-use-outside-click",
]
...
def test_each_template_has_hook_ts():
    for name in TEMPLATE_DIRS:
        assert (TEMPLATES_DIR / name / "hook.ts").exists(), \
            f"{name}: missing hook.ts"
...
def test_hooks_import_react():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "hook.ts").read_text()
        assert "react" in content.lower(), f"{name}: hook.ts missing react import"

def test_hooks_have_exports():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "hook.ts").read_text()
        assert "export" in content, f"{name}: hook.ts missing export"

def test_hooks_not_empty():
    for name in TEMPLATE_DIRS:
        size = (TEMPLATES_DIR / name / "hook.ts").stat().st_size
        assert size > 200, f"{name}: hook.ts too small ({size} bytes)"
