"""
Tests for Express API Patterns pack. TDD — RED first.
"""
...
def test_each_template_has_example_file():
    for name in TEMPLATE_DIRS:
        ...
        assert has_ts, f"{name}: missing example.ts"
...
def test_examples_mention_express():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "example.ts").read_text()
        assert "express" in content.lower(), f"{name}: missing express reference"

def test_examples_have_exports():
    for name in TEMPLATE_DIRS:
        content = (TEMPLATES_DIR / name / "example.ts").read_text()
        assert "export" in content, f"{name}: missing export"
