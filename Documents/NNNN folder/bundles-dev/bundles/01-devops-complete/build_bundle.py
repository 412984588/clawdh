"""
Build DevOps Complete Bundle ZIP.
Pulls the specified tier ZIP from each sub-product and combines into one bundle.
"""
...
NNNN_ROOT = BUNDLE_DIR.parent.parent.parent  # NNNN folder
DIST_DIR = BUNDLE_DIR.parent.parent / "dist"
BUNDLE_JSON = BUNDLE_DIR / "bundle.json"


def get_source_zip(product: dict) -> Path:
    """Find the ZIP for the requested tier of a product."""
...
def main():
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as out_zf:
        # Add bundle README and bundle.json
        out_zf.write(BUNDLE_DIR / "README.md", "README.md")
        out_zf.write(BUNDLE_JSON, "bundle.json")

        # Add each product's contents in its own subfolder
