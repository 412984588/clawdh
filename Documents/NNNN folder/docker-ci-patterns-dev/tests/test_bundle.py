"""Tests for docker-ci-patterns-dev bundle."""
import json
import zipfile
...
TEMPLATE_IDS = [
    ...
    "06-dockerfile-optimization",
    "07-secrets-management",
    "08-health-monitoring",
    "09-multi-service-stack",
...
def test_example_files_contain_docker():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.ts").read_text()
        assert "docker" in content.lower() or "Docker" in content or "container" in content.lower(), \
            f"example.ts in {tid} missing Docker/container reference"
...
def test_starter_zip_exists():
    assert (DIST / "docker-ci-patterns-starter-v1.0.0.zip").exists()


def test_pro_zip_exists():
    assert (DIST / "docker-ci-patterns-pro-v1.0.0.zip").exists()


def test_starter_zip_has_root_files():
    z = zipfile.ZipFile(DIST / "docker-ci-patterns-starter-v1.0.0.zip")
    names = z.namelist()
...
def test_pro_zip_has_all_templates():
    z = zipfile.ZipFile(DIST / "docker-ci-patterns-pro-v1.0.0.zip")
    names = z.namelist()
    for tid in TEMPLATE_IDS:
