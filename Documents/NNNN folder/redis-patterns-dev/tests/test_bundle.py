"""Tests for redis-patterns-dev bundle."""
import json
import zipfile
...
def test_example_files_contain_redis():
    for tid in TEMPLATE_IDS:
        content = (TEMPLATES / tid / "example.ts").read_text()
        assert "redis" in content.lower() or "Redis" in content or "ioredis" in content.lower(), \
            f"example.ts in {tid} missing Redis reference"
...
def test_starter_zip_exists():
    assert (DIST / "redis-patterns-starter-v1.0.0.zip").exists()


def test_pro_zip_exists():
    assert (DIST / "redis-patterns-pro-v1.0.0.zip").exists()


def test_starter_zip_has_root_files():
    z = zipfile.ZipFile(DIST / "redis-patterns-starter-v1.0.0.zip")
    names = z.namelist()
...
def test_pro_zip_has_all_templates():
    z = zipfile.ZipFile(DIST / "redis-patterns-pro-v1.0.0.zip")
    names = z.namelist()
    for tid in TEMPLATE_IDS:
