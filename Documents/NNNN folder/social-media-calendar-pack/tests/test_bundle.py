"""Tests for social-media-calendar-pack bundle."""
import json
import zipfile
...
CONTENT_KEYWORDS = ["content", "social", "post", "audience", "engagement", "platform", "calendar", "strategy"]
...
def test_main_files_have_content_keywords():
    for tid in TEMPLATE_IDS:
        ...
        assert any(kw in content for kw in CONTENT_KEYWORDS), \
            f"template.md in {tid} missing social/content/post/audience/engagement keyword"
...
def test_starter_zip_exists():
    assert (DIST / "social-media-calendar-pack-starter-v1.0.0.zip").exists()


def test_pro_zip_exists():
    assert (DIST / "social-media-calendar-pack-pro-v1.0.0.zip").exists()


def test_starter_zip_has_root_files():
    z = zipfile.ZipFile(DIST / "social-media-calendar-pack-starter-v1.0.0.zip")
    names = z.namelist()
...
def test_pro_zip_has_all_templates():
    z = zipfile.ZipFile(DIST / "social-media-calendar-pack-pro-v1.0.0.zip")
    names = z.namelist()
    for tid in TEMPLATE_IDS:
