...
describe("@voice-hub/claude-marketplace", () => {
  ...
  describe("CLAUDE_CODE_PLUGIN", () => {
    ...
    it("should have settings defined", () => {
      ...
      expect(
        providerSetting?.options.some((o: any) => o.value === "qwen-dashscope"),
      ).toBe(true);
      expect(
        providerSetting?.options.some(
          (o: any) => o.value === "openai-realtime",
        ),
      ).toBe(true);
    });
  ...
  describe("exports", () => {
    it("should export MANIFEST as default", async () => {
      const manifestModule = await import("../src/index.ts");
