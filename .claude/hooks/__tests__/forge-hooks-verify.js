#!/usr/bin/env node
// forge-hooks-verify.js — Forge v2.0 hooks 验证脚本（145 场景）
// 纯 Node.js，无外部依赖
...
// ─────────────────────────────────────────────────────────────────────────────
// 组 27: Codex 后续发现 — partial staging 保护
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n【组 27】Codex 后续修复：partial staging 保护');

// T-11 (source check): partialStagingConflicts 计算逻辑存在
test('27-T11: forge-git-worker.js 包含 partialStagingConflicts 逻辑', () => {
  const src = fs.readFileSync(path.join(HOOKS_DIR, 'forge-git-worker.js'), 'utf8');
  assert(src.includes('partialStagingConflicts'), 'PS-FIX: 应包含 partialStagingConflicts 变量');
  assert(
    src.includes('allStaged.filter(f => touchedRel.includes(f))'),
    'PS-FIX: partialStagingConflicts 应为 allStaged 与 touchedRel 的交集'
  );
  assert(
    src.includes('touchedFilesForAdd'),
    'PS-FIX: 应用 touchedFilesForAdd 替代原始 touchedFiles 调用 safeGitAdd'
  );
  assert(
    src.includes('partialStagingConflicts.includes(rel)'),
    'PS-FIX: touchedFilesForAdd 过滤中应排除 partialStagingConflicts 中的文件'
  );
});

// T-12 (source check): partial staging 冲突时记录日志
test('27-T12: forge-git-worker.js partial staging 冲突记录 logHookEvent', () => {
  const src = fs.readFileSync(path.join(HOOKS_DIR, 'forge-git-worker.js'), 'utf8');
  assert(
    src.includes('partial_staging_skipped'),
    'PS-FIX: partial staging 冲突时应通过 logHookEvent 记录 partial_staging_skipped 事件'
  );
});
...
if (failures.length > 0) {
  ...
  console.log('  全部通过！Forge v2.0 hooks Round 3+Codex 验证完成（145 场景）。');
  console.log('');
  process.exit(0);
}
