...
1. **TDD** — 先写测试再写功能（tdd-enforce.sh + auto-test.sh 强制）
...
- forge-context-save.js 在 35% 自动 git commit 保存进度
- forge-context-save.js 在 25% 自动写 HANDOFF.json + 快照
...
/forge（主工作流）
/forge:status（项目状态）
/gsd:quick（快速小任务，不走完整 Forge 流水线）
