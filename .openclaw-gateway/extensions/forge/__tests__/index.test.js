...
describe('forge internals', () => {
  ...
  test('forge_run remains stable across repeated calls on same project', async () => {
    ...
    const originalHome = process.env.HOME;
    const originalStateDir = process.env.OPENCLAW_STATE_DIR;
    const tempHome = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'forge-run-repeat-'));
    process.env.HOME = tempHome;
    process.env.OPENCLAW_STATE_DIR = path.join(tempHome, '.forge-state');
  ...
  test('forge_implement reuses running execution on repeated calls', async () => {
    ...
    const originalHome = process.env.HOME;
    const originalStateDir = process.env.OPENCLAW_STATE_DIR;
    const tempHome = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'forge-impl-repeat-'));
    process.env.HOME = tempHome;
    process.env.OPENCLAW_STATE_DIR = path.join(tempHome, '.forge-state');
    ...
    try {
      ...
      const next = await tools.get('forge_next').execute('t', { projectId: init.projectId, count: 1 });
      expect(next.success).toBe(true);
      const featureId = next.readyFeatures?.[0]?.id;
      expect(featureId).toBeDefined();

      const first = await tools.get('forge_implement').execute('t', { featureId });
      const second = await tools.get('forge_implement').execute('t', { featureId });
